...
setMethod("do_dmapply",signature(driver="ParallelDDS",func="function",MoreArgs="list", FUN.VALUE="ANY"), 
  function(driver,func,...,MoreArgs=list(), FUN.VALUE=NULL, nparts = NULL){
  dots <- list(...)
  dlen<-length(dots)
  ...
  for(num in 1:dlen){
   ...
   if(type=="list"){
     ...
     if(is.null(nparts)){
         ...
         #Now use nparts to correctly calculate the dimensions
         if(prod(nparts) != length(psizes)) stop("Number of partitions generated by dmapply do not match those supplied in argument 'nparts'.")

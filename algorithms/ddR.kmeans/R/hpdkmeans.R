        ...
        sampling_threshold = 1e6, trace = FALSE, na_action = c("exclude","fail"), completeModel = FALSE)
{
    ...
    #ddyn.load("ddR.kmeans")
    # unloading the so library from all available executors at the exit time
    #on.exit(ddyn.unload("ddR.kmeans"))
    a <- dmapply(function(x) library("ddR.kmeans"), 1:totalParts(X))
...
#O        if (centers == 1) nmeth <- 3
	...
	{
            if(class(Xi) == "matrix")
                .Call("calculate_norm", Xi, Ni, PACKAGE="ddR.kmeans")
            else
                .Call("calculate_norm", as.matrix(Xi), Ni, PACKAGE="ddR.kmeans")
	    return(Ni)
	}
    	...
    	calculateNorm <- function(Xi, Ni)
	{
            good <- maski > 0
            if(class(Xi) == "matrix")
                Ni[good,] <- .Call("calculate_norm", Xi[good,], Ni[good,], PACKAGE="ddR.kmeans")
            else
                Ni[good,] <- .Call("calculate_norm", as.matrix(Xi[good,]), Ni[good,], PACKAGE="ddR.kmeans")
	    return(Ni)
	}
...
.do_oneSet <- function (nmeth, X, Norms, k, centers, iter.max, trace, mask = NULL, completeModel=FALSE) {
    ...
    if(is.null(mask)) {
		...
		cl = integer(nrow(Xi))
                ...
                if(class(Xi) == "matrix")
                    .Call("hpdkmeans_Lloyd", Xi, Ni, centers, cl, nc, PACKAGE="ddR.kmeans")
                else    # When X is sparse, class(Xi) == "dgCMatrix"
                    .Call("hpdkmeans_Lloyd", as.matrix(Xi), Ni, centers, cl, nc, PACKAGE="ddR.kmeans")
...
#print(Sys.time() - timing_info)
		...
		cl = integer(nrow(Xi))
                nc = integer(nrow(centers))
                if(class(Xi) == "matrix")
                    .Call("hpdkmeans_Lloyd", Xi[good,], Ni[good,], centers, cl, nc, PACKAGE="ddR.kmeans")
                else    # When X is sparse, class(Xi) == "dgCMatrix"
                    .Call("hpdkmeans_Lloyd", as.matrix(Xi[good,]), Ni[good,], centers, cl, nc, PACKAGE="ddR.kmeans")
		centers[is.nan(centers)] <- 0
		if(completeModel)
...
print(Sys.time() - timing_info)
    ...
    if(completeModel) {
		...
		wssFunction<- function(Xi, cluster_infoi, centers = centers)
		{
			...
			if(class(Xi) == "matrix")
				dwssi <- .Call("calculate_wss", Xi, centers, clusteri, PACKAGE="ddR.kmeans")
			else
				dwssi <- .Call("calculate_wss", as.matrix(Xi), centers, clusteri, PACKAGE="ddR.kmeans")
	    		dwssi
		}
		...
		wssFunction<- function(Xi, maski, cluster_infoi, centers = centers)
		{
			...
			clusteri = cluster_infoi$clusteri
			if(class(Xi) == "matrix")
				dwssi <- .Call("calculate_wss", Xi[good,], centers, clusteri, PACKAGE="ddR.kmeans")
			else
				dwssi <- .Call("calculate_wss", as.matrix(Xi[good,]), centers, clusteri, PACKAGE="ddR.kmeans")
	    		dwssi
		}
...
hpdapply <- function(newdata, centers, trace=FALSE) {
  ...
  if(darrayInput) { # newdata is a darray
    ...
    if(is.null(mask)) {
	    ...
	    good <- maski > 0
            if(class(Xi) == "matrix")
                .Call("calculate_norm", Xi[good,], Ni[good,], PACKAGE="ddR.kmeans")
            else
                .Call("calculate_norm", as.matrix(Xi[good,]), Ni[good,], PACKAGE="ddR.kmeans")
	    Ni
	}
    	...
    	kmeansFunc <- function(xi, maski, Ni, clusteri, centers)
	{
          ...
          nc = integer(nrow(centers))
          if(m == 1)
            .Call("hpdkmeans_Lloyd", matrix(xi[good,],1), matrix(Ni[good,],1), centers, cl, nc, PACKAGE="ddR.kmeans")
          else    
            if(class(xi) == "matrix")
                .Call("hpdkmeans_Lloyd", xi[good,], Ni[good,], centers, cl, nc, PACKAGE="ddR.kmeans")
            else # When newdata is sparse, class(xi) == "dgCMatrix"
    ...
    nc = integer(nrow(centers))
    if(missingSamples == 0) {
        .Call("calculate_norm", newdata, normsTemp, PACKAGE="ddR.kmeans")
        .Call("hpdkmeans_Lloyd", newdata, normsTemp, centersTemp, cl, nc, PACKAGE="ddR.kmeans")
        cluster <- matrix(cl,nrow=nSample, ncol=1)
    } else {
        if(nSample == 1) {
            normsTemp[mask,] <- .Call("calculate_norm", matrix(newdata[mask,],1), normsTemp[mask,], PACKAGE="ddR.kmeans")
            .Call("hpdkmeans_Lloyd", matrix(newdata[mask,],1), normsTemp[mask,], centersTemp, cl, nc, PACKAGE="ddR.kmeans")
        } else {
            normsTemp[mask,] <- .Call("calculate_norm", newdata[mask,], normsTemp[mask,], PACKAGE="ddR.kmeans")
            .Call("hpdkmeans_Lloyd", newdata[mask,], normsTemp[mask,], centersTemp, cl, nc, PACKAGE="ddR.kmeans")
        }
        cluster[mask,1] <- cl
    }
    if (trace) {    # timing end

...
"hpdRF_parallelForest.default"  <-
    function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500,
             mtry=if (!is.null(y) && !is.factor(y) && !dds::is.dframe(y))
             max(floor(ncol(x)/3), 1) else floor(sqrt(ncol(x))),
             replace=TRUE, classwt=NULL, cutoff, strata,
             sampsize = if (replace) nrow(x) else ceiling(.632*nrow(x)),
             nodesize = if (!is.null(y) && !is.factor(y) && !dds::is.dframe(y)) 5 else 1,
    ...
    if(dds::is.dframe(x)) { # when x is dframe
        if (missing(formula))
            formula <- ~.
        if (!is.null(y))
...
#A        inputData$oob.prox <- oob.prox
    ...
    ## Case 1- compatible to their types in randomForest
        # validating xtest
        if(!is.null(xtest)) {
            if(dds::is.darray(xtest) || dds::is.dframe(xtest) || dds::is.dlist(xtest))
                stop("The type of 'xtest' should be consistent with 'x'")
        }
        # validating y
        if(!is.null(y)) {
            if(dds::is.darray(y) || dds::is.dframe(y) || dds::is.dlist(y))
                stop("'y' cannot be a distributed type when 'x' is not")
        }
        # validating ytest
        if(!is.null(ytest)) {
            if(dds::is.darray(ytest) || dds::is.dframe(ytest) || dds::is.dlist(ytest) || is.null(y))
...
predict.hpdRF_parallelForest <- function (object, newdata, trace=FALSE) {
    ...
    if (!dds::is.darray(newdata) && !dds::is.dframe(newdata)) {
        ...
        if((object$type == "classification") || dds::is.dframe(newdata)) { # the output will be a dframe; either because the output is categorical or to be consistent with the input
            have.dframe = TRUE

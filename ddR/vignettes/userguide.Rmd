---
title: "ddR README"
author: "Edward Ma and Indrajit Roy"
date: "`r Sys.Date()`"
...
ddR is both an API and an R package that permits the declaration of 'distributed' objects (i.e., `dlist`, `dframe`, `darray`), and facilitates parallel operations on these data structures using R-style `apply` functions.
...
A `dlist` may be created using the constructor with a comma-separate list of arguments. For example:
...
The constructors for `darray` and `dframe` are similar but also slightly different. For example, you could initialize a `darray` in the following manner:
